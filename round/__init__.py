...
def _gen_round_to_int(x, *, signature):
    ...
    significand = _reduce(sign, quarters, rounding_mode=signature)
    return to_type_of(0, sign, significand, exponent)


def _gen_round(x, ndigits=None, *, signature):
    """
    Round x to the nearest multiple of a given power of 10.
    """
    if ndigits is None:
...
def round_to_figures(x, figures, *, mode=TIES_TO_EVEN):
    ...
    """
    if figures <= 0:
        raise ValueError("figures should be positive")
